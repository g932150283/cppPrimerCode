...
// 按书名查询图书
int findBooksByName(struct book library[], int total, char searchName[]);
// 按作者查询图书
int findBooksByAuthor(struct book library[], int total, char searchAuthor[]);
// 按书名查询图书
...
void searchBooksByName(struct book library[], int total) {
    ...
    int found = findBooksByName(library, total, searchName);
    if (found == 0) {
        printf("未找到匹配的书籍。\n");
    }else{
        printf("共%d条查询结果。\n", found);
...
void searchBooksByAuthor(struct book library[], int total) {
    ...
    int found = findBooksByAuthor(library, total, searchAuthor);
    if (found == 0) {
        printf("未找到匹配的书籍。\n");
    }else{
        printf("共%d条查询结果。\n", found);
